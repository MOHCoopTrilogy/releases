<!-- GENERATED FILE - DO NOT EDIT.
     Produced by docs/tools/gen_public_pages.py from buglog.json, challenges.scr,
     the map scripts and the authored docs. Edits here are overwritten. -->

# Challenges

Every challenge in the mod, grouped exactly as the in-game **Service Record** groups them (Join Game -> Service Record). Challenges marked *(Elite)* are the harder second tier of the same weapon or feat.

**344 challenges** across **11 categories**.

| Category | Challenges |
|---|---:|
| [RIFLES](#rifles) | 27 |
| [MARKSMAN](#marksman) | 18 |
| [SUBMACHINE GUNS](#submachine-guns) | 18 |
| [SIDEARMS](#sidearms) | 27 |
| [SUPPORT WEAPONS](#support-weapons) | 18 |
| [COMBAT](#combat) | 28 |
| [FIRETEAM](#fireteam) | 21 |
| [ARMOR & VEHICLES](#armor--vehicles) | 17 |
| [CAMPAIGN](#campaign) | 93 |
| [STEALTH & FEATS](#stealth--feats) | 54 |
| [AXIS FORCES](#axis-forces) | 23 |

## RIFLES

| Challenge | How to earn it |
|---|---|
| **Greatest Battle Implement** | Get 50 kills with the M1 Garand |
| **The Tell-Tale Ping** | Get 150 kills with the M1 Garand (Elite) |
| **Their Own Mauser** | Get 50 kills with the Kar98k |
| **Mauser Meister** | Get 150 kills with the Kar98k (Elite) |
| **Hitler's Semi-Auto** | Get 40 kills with the Gewehr 43 |
| **Gewehr Grognard** | Get 120 kills with the Gewehr 43 (Elite) |
| **The Mad Minute** | Get 40 kills with the Lee-Enfield |
| **Bolt-Action Ballet** | Get 120 kills with the Lee-Enfield (Elite) |
| **Rifle of the Rodina** | Get 40 kills with the Mosin-Nagant |
| **For the Motherland** | Get 120 kills with the Mosin-Nagant (Elite) |
| **Tokarev's Answer** | Get 30 kills with the SVT-40 |
| **Red Army Semi** | Get 90 kills with the SVT-40 (Elite) |
| **Mannlicher's Cast-Off** | Get 30 kills with the Carcano |
| **Il Fucile** | Get 90 kills with the Carcano (Elite) |
| **Chrysanthemum Stock** | Get 30 kills with the Arisaka |
| **Banzai Bolt** | Get 90 kills with the Arisaka (Elite) |
| **Sturmgeist's Choice** | Get 40 kills with the StG44 |
| **Sturm und Drang** | Get 120 kills with the StG44 (Elite) |
| **Sniper's Eye** | Get 100 kills with the M1 Garand |
| **Quiet Ping** | Get 175 kills with the M1 Garand |
| **Mauser Master** | Get 100 kills with the Kar98k |
| **Walther's Finest** | Get 100 kills with the Gewehr 43 |
| **Sniper's Enfield** | Get 100 kills with the Lee-Enfield |
| **Rodina's Rifle** | Get 100 kills with the Mosin-Nagant |
| **Cecchino** | Get 100 kills with the Carcano |
| **Palm Nest** | Get 100 kills with the Arisaka |
| **Sturm Scope** | Get 100 kills with the StG44 |

## MARKSMAN

| Challenge | How to earn it |
|---|---|
| **Old Reliable** | Get 40 kills with the Springfield '03 |
| **One Shot, One Kill** | Get 120 kills with the Springfield '03 (Elite) |
| **Their Own Scope** | Get 40 kills with the Kar98 Sniper |
| **Scharfschutze** | Get 120 kills with the Kar98 Sniper (Elite) |
| **Zielfernrohr** | Get 30 kills with the G43 Sniper |
| **Semi-Auto Assassin** | Get 90 kills with the G43 Sniper (Elite) |
| **Zaitsev's Trade** | Get 30 kills with the Mosin Sniper |
| **Ghost of Stalingrad** | Get 90 kills with the Mosin Sniper (Elite) |
| **Mannlicher's Eye** | Get 25 kills with the Carcano Sniper |
| **Il Cecchino** | Get 75 kills with the Carcano Sniper (Elite) |
| **Nest in the Palms** | Get 25 kills with the Arisaka Sniper |
| **Jungle Ghost** | Get 75 kills with the Arisaka Sniper (Elite) |
| **His Majesty's Marksman** | Get 25 kills with the L42A1 |
| **Enfield Enforcer** | Get 75 kills with the L42A1 (Elite) |
| **The Silent Carbine** | Get 25 kills with the De Lisle |
| **Whisper of Death** | Get 75 kills with the De Lisle (Elite) |
| **Every Window a Rifle** | Get 10 sniper-rifle kills in Sniper Town |
| **Bolt Action Only** | Finish Stavelot with nothing but a bolt-action rifle |

## SUBMACHINE GUNS

| Challenge | How to earn it |
|---|---|
| **The Chopper** | Get 50 kills with the Thompson |
| **Chicago Piano** | Get 150 kills with the Thompson (Elite) |
| **GI's Grease Gun** | Get 40 kills with the M3 Grease Gun |
| **Cheap and Deadly** | Get 120 kills with the M3 Grease Gun (Elite) |
| **Not a Schmeisser** | Get 50 kills with the MP40 |
| **Burp Gun Baron** | Get 150 kills with the MP40 (Elite) |
| **Plumber's Nightmare** | Get 40 kills with the Sten |
| **Woolworth Wonder** | Get 120 kills with the Sten (Elite) |
| **Papasha** | Get 40 kills with the PPSh-41 |
| **Drum of the Red Army** | Get 120 kills with the PPSh-41 (Elite) |
| **Nambu's Chatterbox** | Get 30 kills with the Type 100 |
| **Rising Sun Ripper** | Get 90 kills with the Type 100 (Elite) |
| **Balilla Automatico** | Get 30 kills with the Beretta M38 |
| **Roman Ripper** | Get 90 kills with the Beretta M38 (Elite) |
| **More Dakka** | Get 100 kills with the Thompson |
| **Hush Puppy** | Get 100 kills with the MP40 |
| **Silent Papasha** | Get 100 kills with the PPSh-41 |
| **Oiled and Quiet** | Get 100 kills with the Grease Gun |

## SIDEARMS

| Challenge | How to earn it |
|---|---|
| **Cocked and Locked** | Get 40 kills with the Colt .45 |
| **Slabsides Slayer** | Get 120 kills with the Colt .45 (Elite) |
| **Wet Work** | Get 30 kills with the Silenced Pistol |
| **OSS Special** | Get 90 kills with the Silenced Pistol (Elite) |
| **Spoils of Rank** | Get 40 kills with the Walther P38 |
| **Officer's Sidearm** | Get 120 kills with the Walther P38 (Elite) |
| **The Parabellum Prize** | Get 30 kills with the Luger P08 |
| **Toggle-Lock Trophy** | Get 90 kills with the Luger P08 (Elite) |
| **Gestapo Pocket** | Get 30 kills with the Walther PPK |
| **Kripo's Companion** | Get 90 kills with the Walther PPK (Elite) |
| **Six for the Crown** | Get 30 kills with the Webley |
| **Top-Break Tommy** | Get 90 kills with the Webley (Elite) |
| **Seven for Stalin** | Get 30 kills with the Nagant Revolver |
| **Commissar's Companion** | Get 90 kills with the Nagant Revolver (Elite) |
| **Tokarev Steel** | Get 30 kills with the TT-33 |
| **Soviet Sidearm** | Get 90 kills with the TT-33 (Elite) |
| **Bella Pistola** | Get 30 kills with the Beretta |
| **Roman Roscoe** | Get 90 kills with the Beretta (Elite) |
| **Papa Nambu** | Get 30 kills with the Nambu |
| **Rising Sun Roscoe** | Get 90 kills with the Nambu (Elite) |
| **Assassin's Whisper** | Get 25 kills with the Welrod |
| **SOE Silencer** | Get 75 kills with the Welrod (Elite) |
| **Old Slabsides** | Get 50 kills with the Colt .45 |
| **Officer's Whisper** | Get 50 kills with the Walther P38 |
| **Toggle Silencer** | Get 50 kills with the Luger P08 |
| **Soviet Silencer** | Get 50 kills with the TT-33 |
| **Bella Silenziosa** | Get 50 kills with the Beretta |

## SUPPORT WEAPONS

| Challenge | How to earn it |
|---|---|
| **Walking Fire** | Get 50 kills with the BAR |
| **Browning's Bruiser** | Get 150 kills with the BAR (Elite) |
| **Belt-Fed Fury** | Get 40 kills with the .30 Cal |
| **Mow 'Em Down** | Get 120 kills with the .30 Cal (Elite) |
| **Water-Cooled Wrath** | Get 30 kills with the Vickers |
| **Somme Survivor** | Get 90 kills with the Vickers (Elite) |
| **Modello 30 Misery** | Get 30 kills with the Breda 30 |
| **Italian Iron** | Get 90 kills with the Breda 30 (Elite) |
| **Bonesaw** | Get 40 kills with the MG42 |
| **Hitler's Zipper** | Get 120 kills with the MG42 (Elite) |
| **Diplomatic Protest** | Get 30 kills with the Shotgun |
| **Trench Sweeper** | Get 90 kills with the Shotgun (Elite) |
| **Bob Burns' Horn** | Get 15 kills with the Bazooka |
| **Stovepipe Serenade** | Get 45 kills with the Bazooka (Elite) |
| **Tank Terror** | Get 15 kills with the Panzerschreck |
| **Ofenrohr Overkill** | Get 45 kills with the Panzerschreck (Elite) |
| **Spring-Loaded Suicide** | Get 15 kills with the PIAT |
| **Blacker's Bombard** | Get 45 kills with the PIAT (Elite) |

## COMBAT

| Challenge | How to earn it |
|---|---|
| **Kopfschuss** | Get 25 headshot kills |
| **Kopfjager** | Get 100 headshot kills |
| **Cold Steel** | Get 15 melee kills |
| **Knuckle and Blade** | Get 45 melee kills |
| **Scripted Havoc** | Kill 2 or more enemies with a single exploding barrel |
| **Through the Bocage** | Kill 10 enemies with a wallbang shot |
| **Kentucky Windage** | Get 25 long-range kills (over 2000 units) |
| **A Thousand Yards** | Get 75 long-range kills |
| **Blind Faith** | Get 15 blindfire kills from cover |
| **Welcoming Committee** | Kill 30 enemy reinforcements |
| **Fire in the Hole** | Get 20 grenade kills |
| **Granate!** | Get 60 grenade kills |
| **Backblast Clear** | Get 15 rocket or bazooka kills |
| **Taking You With Me** | Kill 10 enemies while downed (DBNO) |
| **Blooded** | Get 100 total kills |
| **Business Is A-Boomin'** | Business is a-boomin' - get 250 total kills |
| **Dogface** | Get 1000 total kills |
| **To Hell and Back** | Get 2500 total kills |
| **Todesengel** | Get 5000 total kills |
| **A Persistent One...** | Eliminate your first High-Ranking Officer |
| **Herr Kommandant** | Kill 5 enemy officers |
| **Enjoy the Walk** | Kill 20 enemy officers |
| **Behind the Sandbags** | Get 25 kills from a cover pose or deployed cover |
| **Fire and Movement** | Get 100 kills from a cover pose or deployed cover (Elite) |
| **Dug In** | Get 10 kills from behind cover you deployed yourself |
| **Hoist by His Own Petard** | Make an officer blow himself up with one of his own reinforcements |
| **The Full Deck** | Face down all eight kinds of reinforcement the Officer can call |
| **Not Today, Fritz** | Survive a Stuka dive and an artillery barrage on the same map without going down |

## FIRETEAM

| Challenge | How to earn it |
|---|---|
| **Leave No Man** | Save a teammate 20 times with a savior kill |
| **For Donnie** | Get 60 savior kills |
| **Sulfa and a Prayer** | Revive 10 downed teammates |
| **Corpsman Up!** | Revive 40 downed teammates |
| **Fliegeralarm!** | Call in your first bombing run |
| **On My Smoke** | Call in 5 air strikes |
| **Steel Rain** | Call in 20 air strikes |
| **Anywhere on The Map** | Call in your first paratrooper drop |
| **Stand in the Door** | Call in 5 paradrops |
| **Currahee** | Call in 15 paradrops |
| **Freeze-Tag Only** | Revive teammates three times in a single level |
| **Quartermaster** | Resupply teammates 25 times from ammo boxes you placed |
| **All Present and Correct** | Finish a mission with four players alive and nobody ever downed |
| **Field Promotion** | Revive the same teammate three times in one mission |
| **Simultaneous Salvation** | Two teammates revived within ten seconds of each other |
| **Ring of Steel** | Have the whole squad dug into your deployed cover when the officer calls a wave |
| **Four Abreast** | Reach the Omaha seawall with all four players still alive |
| **Corpsman, Forward** | Revive 10 downed allied soldiers |
| **Nobody Bled Out** | Finish a mission where every ally who went down was picked back up |
| **Field Hospital** | Revive an ally while you are taking fire yourself |
| **Matching Set** | Finish a mission with the whole squad wearing the same helmet |

## ARMOR & VEHICLES

| Challenge | How to earn it |
|---|---|
| **Panzerknacker** | Destroy 10 enemy vehicles in the campaign |
| **Panzer Graveyard** | Destroy 30 enemy vehicles in the campaign |
| **Not One Stone** | Destroy the tank before it lands a single hit on the farmhouse |
| **Day of the Tiger** | Defend The Bridge keeping the King Tiger at 85% health or above |
| **Tiger by the Tail** | Knock out the enemy King Tiger at the bridge |
| **Sticky End** | Kill a Tiger tank with a sticky bomb |
| **Red Ball Express** | Get the supply truck to the Allied line intact |
| **The Chancellery Job** | Crack the Chancellery safe and steal a T-34 |
| **Chasing the Fox** | Win the Panzer duel on the Tunisian front |
| **Autoblinda Down** | Destroy the Italian AB-41 with the Flak cannon |
| **Two More for the Scrapheap** | Find and bomb both hidden King Tigers in the Gestapo motor pool |
| **Pole Position** | Flatten all three telephone poles with the King Tiger |
| **Turnabout** | Destroy the halftrack blocking the Ardennes road |
| **Not One Step Back** | Hold the last bridge without going down |
| **Save the Crab** | Bring the minesweeper tank through Bizerte alive |
| **Nine Tanks, No Ground** | Repel the tank assault with nobody going down |
| **Nothing Gets Through** | Destroy the munitions convoy without ever raising the alarm |

## CAMPAIGN

| Challenge | How to earn it |
|---|---|
| **The Business We're In** | Complete 15 primary objectives |
| **Conspicuous Gallantry** | Complete 50 primary objectives |
| **Dead Air** | Complete 10 side objectives |
| **Easy Red** | Complete the Omaha Beach landing |
| **Beyond the Seawall** | Complete the D-Day breakout |
| **Iron Coffin** | Complete the U-boat pens |
| **The Marshalling Yard** | Complete the train station finale |
| **Torchbearer** | Complete Lighting the Torch |
| **Hell's Highway** | Hold the bridge (Operation Market Garden) |
| **The Ghost Front** | Survive the Ardennes counteroffensive |
| **Red Bull Country** | Hold the line in Tunisia |
| **Wet Behind the Ears** | Complete 1 mission |
| **Combat Ready** | Complete 5 missions |
| **The Road to Berlin** | Complete 15 missions |
| **The Guns Fall Silent** | Complete 30 missions (full campaign) |
| **Hell Let Loose** | Wipe out an officer, his bodyguards and the battalion without going down |
| **Blue Fox, This Is White Rook** | Infiltrate the sub pen and finish the mission for Grillo |
| **On Her Majesty's Secret Service** | Help Major Grillo steal the German identification papers |
| **Gamespy** | Recover either set of stolen papers on the Naxos mission |
| **Expect Close Contact** | In the Bocage house, keep an airborne soldier alive through both ambushes |
| **The Way to Win a War** | Keep Captain Ramsey alive through the Nebelwerfer Hunt |
| **Tooth and Nail** | Eliminate every sniper in Sniper Town |
| **Steady the Line** | Steal the King Tiger with Sergeant Hammon still standing |
| **Combat Medic** | Steal the King Tiger with T/4 Medic Glenn still standing |
| **Sapper's Luck** | Steal the King Tiger with T/2 Engineer Campbell still standing |
| **Brothers in Arms** | Finish the Normandy bridge raid with the British captain still at your side |
| **The Colonel's Reckoning** | Track down and eliminate Colonel Hildebrandt |
| **No Man Left** | Finish the Normandy bridge raid with the private still at your side |
| **Rescue Detail** | Locate the wounded captain at Bastogne and finish the mission with him alive |
| **By the Book** | Finish the Bastogne hotel siege with the captain still standing |
| **The Double Agent** | Finish the Bizerte Canal mission with Claus still alive |
| **Briefing Room** | Finish the Bizerte Canal mission with Captain Burton still standing |
| **Row, Row** | Finish the Bizerte Canal mission with Gobbs still standing |
| **Doc at the Wheel** | Finish the Italy jeep run with Whittaker still alive |
| **Welded Steel and Storms** | Acquire the StG44 blueprints |
| **That One Map No One Played** | Board the train on The Schmerzen Express |
| **Bring the Bocage Boy Home** | Keep Lyndon alive through the farmhouse tank assault |
| **Bring the Boys Back Home** | Keep Johnson and Hudson alive through the end of the mission |
| **A Boy to do a Man's Job** | Hold off the tanks with no one dying |
| **First of Its Kind** | Enter the armory and steal an StG44 |
| **Finest Hour** | Reach the bangalores on Omaha without a squadmate going down |
| **Rudder's Rangers** | Silence every gun emplacement above Omaha Beach |
| **Room by Room** | Clear the strongpoint house with nobody downed |
| **Silence the Acht-Acht** | Destroy both Flak 88s in the D-Day breakout |
| **Hitler's Buzzsaw** | Hold the MG42 through the reinforcement wave, nobody downed |
| **Stick Close to Me** | Get the SAS agent out of the fortress alive |
| **Blind and Deaf** | Destroy every piece of enemy communications equipment |
| **Light the Beacon** | Light the lighthouse beacon and signal the fleet |
| **The Naxos Papers** | Steal the research documents from the Naxos complex |
| **Blind the Wolfpack** | Destroy the Naxos radar prototype |
| **All Present and Accounted For** | Reach the train-station rendezvous with the whole squad up |
| **Get the Flyboy Home** | Escort the downed pilot to the Maquis hideout alive |
| **Beutepanzer** | Steal the King Tiger with the tank crew still alive |
| **Drive It Like You Stole It** | Break out of Sniper Town with the squad |
| **Static Line Savior** | Save the trooper hanging from the power pole on Crete |
| **Not One Yard Back** | Hold the line at Bastogne without a single breach |
| **Held the Line** | Hold Bastogne with no breach AND the whole squad still alive |
| **Every Position Searched** | Search every crew position in Sniper Town before the bazooka appears |
| **Crew Intact to City Hall** | Reach city hall with the whole tank crew still alive |
| **Valve by Valve** | Shut down both valves in the Fort Schmerzen gas works |
| **Quad-Twenty** | Destroy the 20mm Flaks in the frozen forest |
| **Fort of Pain** | Plant the charges deep in the Fort Schmerzen gas works |
| **Out Through the Fire** | Escape the burning fort with everyone on their feet |
| **Where Patterson Fought** | Complete the Fort Schmerzen finale |
| **Crossing the Waal** | Destroy both Flak 88s guarding the bridge town |
| **Kill the Screaming Mimis** | Destroy all five Nebelwerfers in the Ardennes |
| **The Poor Bastards** | Survive all three Bastogne defense waves, nobody downed |
| **Nothing Quiet About It** | Clear the church and hotel at Stavelot |
| **Hold Until Relieved** | Hold the last bridge until air support arrives |
| **Ground the Luftwaffe** | Sabotage the enemy airfield |
| **Anzio Annie** | Destroy the K5 railway guns |
| **The Stranded Whale** | Call in the Anzio airstrike and finish the campaign |
| **Alarm! Alarm!** | Board U-529 with the base on full alert |
| **All Hands on Deck** | Clear the deck of U-529 from aboard her, with nobody going down |
| **The Final Broadcast** | Use the call sign to divert enemy reinforcements before the blackout |
| **Stealing the Show** | Go loud after detonating the command post and fight your way out |
| **The Whole Section** | Finish Lighting the Torch with Richards, Wilson, Thomas and Allen all still on their feet |
| **Cut Him Down** | Free the interrogation-chair prisoner without him taking a single hit |
| **The Whole Beach** | Take the bunker at the top of Omaha with nobody going down from the ramp to the door |
| **Held Both Doors** | Clear the house and hold both the back and the front without anyone going down |
| **Not a Scratch on Him** | Deliver the downed pilot to the Maquis hideout without him taking a single hit |
| **The Bridge Still Stands** | Finish The Bridge without the bridge going down, theirs or yours |
| **Not One Ranger** | Clear the boxcar sniper duel at Fort Schmerzen without losing a single man |
| **Ahead of the Flames** | Escape Fort Schmerzen in under six minutes |
| **Both Boys Home** | Take the Flak 88 town with neither of your riflemen lost |
| **Four Charges, One Bridge** | Set every charge on the bridge and be back with the Captain before they blow |
| **Three Came Back** | Finish the Ardennes with nobody going down |
| **Both Guns Turned** | Seize both Flak 88 batteries above Kasserine Pass |
| **Every Man Out** | Break out of the Bizerte jail with nobody going down |
| **All Six Home** | Finish Crete with Hudson, McMartin, Phillips, Johnson, Gobs and Michaels all alive |
| **All Seven Home** | Cover the prisoners' escape from Monte Battaglia without losing one |
| **Able, Baker, Charlie** | Hold the bunkers below the castle without going down |
| **Spotless Record** | Complete 5 missions with every primary objective done and nobody going down |

## STEALTH & FEATS

| Challenge | How to earn it |
|---|---|
| **Pistol Pete** | Beat a campaign level using only your sidearm (Colt .45 or equivalent) |
| **I've Seen French Schoolgirls Shoot Better!** | Finish a level without ADS, a scope, or the over-shoulder aim camera |
| **Stalingrad Rifle-Only Server** | Beat a campaign level using only a bolt-action rifle |
| **Kings Push** | Reach the train station on Diverting the Enemy in under five minutes |
| **Earn This.** | Beat the bridge mission (The Bridge) without healing or dying |
| **Wuss.pk3** | Register 700+ distinct sounds in one session - your sound table is nearly full |
| **POOPHEAD ENDING** | Finish a mission the way the developers meant to end it, not the way they shipped it |
| **BreakTheGame.SCR** | Complete all 13 missions where the devs had arguments |
| **PREVENT COMA AFTER 60 SECONDS** | Put down all three patrollers the devs had to force-activate to stop them going catatonic |
| **Look, I Washed For Supper** | Eliminate the retreating Wehrmacht behind the Omaha bunkers before they escape |
| **Silent Night, Deadly Night** | Infiltrate the town and destroy the command post without raising any alarms |
| **Across Dog Green** | Cross the beach to the seawall in under 90 seconds |
| **Silence in the Pens** | Reach the second U-boat without raising the alarm |
| **Eyes of the Milice** | Complete the Paper Chase without tripping the alarm |
| **The Way it Was** | Destroy the U-boat the way it was intended to be |
| **Mit den Fischen schlafen** | Scuttle the U-boat without killing a single man yourself - let the sea take them |
| **Paper Trail** | Recover 5 hidden blueprints |
| **Draughtsman** | Recover 12 hidden blueprints |
| **Archivist** | Recover 20 hidden blueprints |
| **The Whole Portfolio** | Recover all 31 hidden blueprints |
| **Eyes Open** | Clear Sniper Town without ever using a scope or the aim camera |
| **The Long Way** | Hold the bocage house without ever firing a mounted machinegun |
| **Weathered** | Complete a mission in each of the five weather themes |
| **Cut the Hand Short** | End the card game in the U-boat pens the hard way |
| **Wire It Quiet** | Wire the radio command post undetected |
| **Papers, Please** | Infiltrate the ship in disguise without blowing your cover |
| **Shadow over U-529** | Infiltrate U-529 without an alarm ever being raised |
| **Pattern Disrupt** | Successfully contain a situation |
| **Dead Men Tell Tales** | Have a body investigation blow your cover |
| **Dead Static** | Destroy the Naxos without alerting a scientist |
| **OSS Efficiency** | Contain a situation and sabotage the Naxos within 30 seconds, unseen |
| **Thunder / Flash** | Clear the house in the Bocage within 30 seconds |
| **Bad Reception** | Distract an officer with a radio, then contain him |
| **The Storm Rises** | Infiltrate the compound unseen, then leave it in ruins |
| **A Brief Respite** | Take a moment to appreciate what was |
| **Monkey Business** | Destroy Stuka number seven, the one the developers hand-wired |
| **Close Encounters of the Turd Kind** | Catch the one German who never made it off the latrine |
| **The Manifest** | Find the Enigma manifest hidden aboard the U-boat |
| **Ghost in the Ductwork** | Reach the train station without the base ever escalating past its first alert |
| **Rode It In** | Reach the tank park aboard the German truck without ever being spotted |
| **House Call** | Finish Sniper Town with nobody going down |
| **No Bells at Nordwind** | Take the blueprints, the rifle and the stockpile without ever tripping the alarm |
| **Gas Discipline** | Finish the gas mission with nobody going down |
| **Shabbadoo** | Nobody knows what it means. Trip it anyway |
| **Kalimba Says** | Click the thing you were never meant to find |
| **Six Flushes** | Flush the outhouse six times and see what comes out |
| **Sneakers** | Cross the Bizerte patrol ground without ever being seen |
| **Not a Single Bell** | Scuttle the freighter and get clear without the ship alarm ever sounding |
| **They Never Saw Us** | Let the Italian search party finish all four lines at the wrecked glider before you fire |
| **Let Them Sleep** | Sabotage all four fighters without waking a single sleeping guard |
| **Back of the Class** | Find the lesson nobody was supposed to interrupt |
| **Quiet Professionals** | Finish the coastal battery mission without ever raising the alarm |
| **Seconds to Spare** | Get clear of the castle in under eight minutes |
| **The Quiet Type** | Complete five missions without ever raising the alarm |

## AXIS FORCES

| Challenge | How to earn it |
|---|---|
| **Frontline Sweeper** | Eliminate 50 Wehrmacht infantry |
| **Iron Curtain Smasher** | Eliminate 250 Wehrmacht infantry |
| **The Scourge of Berlin** | Eliminate 1000 Wehrmacht infantry |
| **Submarine Saboteur** | Eliminate 15 Kriegsmarine sailors |
| **U-Boat Threat Neutralized** | Eliminate 35 Kriegsmarine sailors |
| **Desert Fox Hunter** | Eliminate 50 Afrika Korps soldiers |
| **Sands of Victory** | Eliminate 150 Afrika Korps soldiers |
| **Know Your Enemy** | Kill at least one soldier from 5 different Axis units |
| **Order of Battle** | Kill at least one soldier from 8 different Axis units |
| **Shadow Hunter** | Eliminate 12 Gestapo officers |
| **Total De-Nazification** | Eliminate 20 Gestapo officers |
| **Anzio Advance** | Eliminate 40 Italian infantry |
| **Apennine Breakthrough** | Eliminate 110 Italian infantry |
| **Avanti!** | Eliminate 40 Italian soldiers of any unit |
| **Armistice Enforced** | Eliminate 120 Italian soldiers of any unit |
| **Town Marshal** | Eliminate 10 Italian militia volunteers |
| **The New Order** | Eliminate 20 Italian militia volunteers |
| **Silencing the Comm-Link** | Eliminate 15 German field officers |
| **Chain of Command Broken** | Eliminate 50 German field officers |
| **Bunker Buster** | Eliminate 4 high-ranking commanders |
| **High-Value Target** | Eliminate 8 high-ranking commanders |
| **Counter-Sniper** | Eliminate 15 enemy snipers |
| **Ghost in the Fog** | Eliminate 45 enemy snipers |

