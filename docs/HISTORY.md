...
- 2026-08-08 (later): U crewed AA emplacements - all three placeable AA guns get an animated gunner, and the two mannable ones hand off to a player on mount and take the crew back on dismount. Unlocked by two verified mechanisms: `QueryTurretSlotEntity 0` addresses a cannon that `spawnturret` created at runtime (retail uses the same idiom at e2l1 `FlakGunSetup`), and the `flak88_driver`/`aagun_driver` poses are in the SHARED human anim set so every model has them. Same session: 4 manned MG42 nests baked into t2l1, `max_health` property fix in the t2l1 tank gag (one token, 268 errors/session), officer radio NIL guard.
- 2026-08-08 (v1.2.3): U allied squads made survivable - health scales with player count and they go DOWN instead of dying, with the player's own DBNO animation and marker, revivable by proximity at no medkit cost (coop_mod/allysquad.scr). That is what made it safe to narrow the engine blast shield from every allied actor to an opt-in flag (bug-1586), so mortars can finally wound and gib allies - the gore system was never the problem, the damage was being dropped before it. Same release: new-objective toast on HUD 135-149, ambient no-damage barrage, crewed AA emplacements, t2l1 given a real 'keep the squad alive' objective plus per-loss warnings, officer disabled on t2l1, 29 OFFICER: dev prints gated, MAX_CVARS 4096->8192 after the Service Record crash (bug-1582).
- **2026-08-10** m2l2a stealth: master plan v2 written and vetted in 3 adversarial rounds (40 agents, ~270 findings). **Phase A shipped** - bug-1631 freeze arm deleted (VERIFIED in play: the papers guard now accepts and stays animated), all six disguise anim gates made per-target and latch-free (they had been unconditionally true - `thread` in a boolean returns a handle), one aggro rule via `attackentity` instead of the one-way `attackplayer` latch, and the scene-actor wreckers exempted after A3 instrumentation MEASURED coop_apply_personality proning the card man, a Naxos watcher and an alarm runner. Full mission completed with zero Script Errors against an empty baseline.
