...
def load_challenges():
    # [user 2026-08-26] challenges.scr is no longer the only definition site. mvchal_gen.scr carries
    # one chal_def per weapon MODEL VARIANT (77 of them, generated by variant_challenges.py). Reading
    # only challenges.scr meant the build gate printed "every challenge has a writer" while never
    # having looked at them - the exact class of false green this checker exists to prevent.
    src = io.open(CHAL, encoding="latin-1").read()
    for extra in ("mvchal_gen.scr",):
        p = os.path.join(MOD, "coop_mod", extra)
        if os.path.exists(p):
            src += chr(10) + io.open(p, encoding="latin-1").read()
    out = []
