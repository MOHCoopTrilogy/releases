...
Sound params are copied verbatim from the retail alias lines with the maps gate STRIPPED, so
every pool loads on every map (we always mount all three games). Rerunnable; output is
...
def add(nat, sit, path, parms):
    parms = re.sub(r'\s*maps\s+"[^"]*"', '', parms).rstrip()
...
L = ["// GENERATED by docs/tools/gen_chatter_pools.py - DO NOT EDIT.",
     "// Cut-content VO folded into the aivoice pools; maps gates stripped (see generator doc).",
     ""]
total = 0
