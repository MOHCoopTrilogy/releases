...
| metric | value |
|---|---:|
| `coop_mod/*.scr` files | 133 |
| total lines in `coop_mod/` | 80,146 |
...
| `mp.scr` | 439 | 2 | MP.SCR - the multiplayer entry point. NOT the coop framework. |
| `mp_armory.scr` | 522 | 5 | MP_ARMORY.SCR - the shared MP armory dispatcher (slice 2, LIVE). |
| `mp_freezetag.scr` | 309 | 2 | MP_FREEZETAG.SCR - FREEZE TAG, an MP game MODE (slice 5). Decisions M12/M13/M14: played on Team |
| `mp_gungame.scr` | 275 | 2 | MP_GUNGAME.SCR - Gun Game, the first MP game MODE (slice 1). FFA (gt1) only for v1 (decision M9). |
