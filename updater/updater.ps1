...
# In-game "Report a Bug": export the webhook into a loose maintt cfg the game execs at launch
# (coop_reportwebhook.cfg). Kept OUT of the public pk3; regenerated here each launch so it tracks
# updater.ini. Empty/missing = the in-game reporter just says "not configured".
try {
    $wh = $ini["ReportWebhook"]
    if ($wh) {
        # [user 2026-08-22, bug-2030] WAS "$app\maintt", which does not exist. The game runs with
        # fs_homepath "$app\home" (see LaunchGame above) and every manifest file downloads to
        # "$app\home\maintt\", so the cfg was written nowhere - Test-Path failed, the `if` skipped
        # silently, and because this whole block sits in a try{} with no else it did not even log.
        # autoexec.cfg execs coop_reportwebhook.cfg faithfully; the file simply never existed, so
        # coop_reportWebhook stayed "" and the IN-GAME reporter was dead for every player since it
        # shipped. Create the directory rather than requiring it: on a fresh install this can run
        # before the first pk3 download.
        $mt = Join-Path $app "home\maintt"
        try { if (-not (Test-Path $mt)) { New-Item -ItemType Directory -Path $mt -Force | Out-Null } } catch {}
        if (Test-Path $mt) {
            Set-Content -Path (Join-Path $mt "coop_reportwebhook.cfg") -Encoding ASCII -Value @(
                "// AUTO-GENERATED by updater.ps1 from updater.ini - Discord webhook for in-game Report a Bug.",
                ('seta coop_reportWebhook "' + $wh + '"'))
            Log "wrote coop_reportwebhook.cfg for in-game reporter"
        } else {
            Log "WARN could not create $mt - in-game reporter will say 'not configured'"
        }
    } else {
        Log "no ReportWebhook in updater.ini - in-game reporter will say 'not configured'"
    }
} catch { Log "WARN webhook seed failed: $_" }
