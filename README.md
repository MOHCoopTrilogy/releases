...
> [!CAUTION]
> **Not yet playtested end to end. Try it at your own risk.** Neither a full multiplayer session
> nor a complete single-player campaign run has been played through and verified on the current
> build. Individual systems are tested as they are built, but the whole thing start-to-finish is
> not — expect to hit things nobody has hit yet. If you do, the Report a Problem tool below is
> genuinely the most useful thing you can send us.
>
...
### Loadout, Armory & unlockables

- A **Loadout screen** where you pick what you carry in, rather than being handed a fixed kit
- The **Armory**: a 69-gun picker covering the whole trilogy's arsenal plus imported extras, with locked entries showing what earns them
- **213 unlocks** earned purely by playing — **29 weapons**, **15 perks**, **48 helmets** and **121 uniforms/skins**. Nothing is bought and nothing is random: every single unlock is attached to a named challenge, so you can always see exactly what to do to get it
- Perks are passive squad abilities and extra equipment (extra grenades, faster revives, more smoke, and so on) rather than stat inflation
- The full chart of what unlocks what is **[here](docs/public/UNLOCKABLES.md)**
...
### Settings you actually get to change

Three menus were added on top of the stock options, because most of the above is worth tuning
rather than forcing on everyone:

- **Advanced Graphics** — the knobs the original game never exposed: texture filtering and anisotropy, LOD and draw distance, detail levels, and the display-mode selector (Borderless or Exclusive Fullscreen). **Use Borderless if you record or stream** — it is capture-safe, and exclusive fullscreen bypasses the desktop compositor so capture tools see a black screen
- **Post-FX** — bloom, contrast-adaptive sharpening, depth of field, and the screen effects for suppression and low health, each toggleable and tunable across three pages. If you prefer the classic look, you can turn the whole layer off
- **Coop Settings** — the gameplay side: HUD behaviour and fade, the XP popup, crosshair and aim options, audio mixer levels, and the coop rules themselves. Reachable in-game from the desk telephone in the menu room, and most of it is per-player rather than server-wide

Server hosts get a further layer through cvars — player health, DBNO, Last Man Standing lives,
corpse persistence, AI scaling caps and more.
